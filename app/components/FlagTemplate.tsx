...
export default function FlagTemplateComponent(props: FlagTemplateProps) {
        ...
        function handleSubmit(event) {
                ...
                newFlag["clientSideAvailability"] = {
                        usingEnvironmentId: (data.get("usingEnvironmentId")?.toString() === 'on'),
                ...
                for (let i = 0; i <= variationCount; i++){
                        ...
                        const variation = {
                                ...
                                value: forceFlagType(data.get("flagType")!.toString(), data.get(`variations${i}.value`)),
                        }
      ...
      <p>
        <label>
          Name: <input name="name" type="text" defaultValue={flagData.name} />
        </label>
        </p>
        <p>
        <label>
          Key: <input name="key" type="text" defaultValue={flagData.key} />
        </label>
        </p>
        <p>
        <label>
          Description: <input name="description" type="text" defaultValue={flagData.description} />
        </label>
        </p>
        <p>
        <label>
          Mark as Permanent: <input name="temporary" type="checkbox" defaultChecked={!flagData.temporary} />
        </label>
        </p>
        <fieldset>
                <legend>Choose Flag Availability</legend>
                <label>Client-Side
                <input type="checkbox" id="usingEnvironmentId" name="usingEnvironmentId" defaultChecked={flagData.availability.client} />
                </label>

                <label>Mobile
                <input type="checkbox" id="usingMobileKey" name="usingMobileKey" defaultChecked={flagData.availability.mobile} />
                </label>
        </fieldset>
        ...
        <fieldset>
                ...
                <label>On Variation:
                <select name="onVariation" id="variation-on" defaultValue={flagData.defaultVariation.onVariation}>
                        {flagData.variations.map((item: Variation, idx: number) => (
                            <option
                                key={item.value}
                                label={item.name ? item.name : JSON.stringify(item.value)}
                                value={idx}
                            />
                        ))}
                    </select>
                ...
                <br />
                <label>Off Variation:
                <select name="offVariation" id="variation-off" defaultValue={flagData.defaultVariation.offVariation}>
                        {flagData.variations.map((item: Variation, idx: number) => (
                            <option
                                key={item.value}
                                label={item.name ? item.name : JSON.stringify(item.value)}
        ...
        {flagData.variations.map((item: Variation, idx: number) => (
                ...
                <label>
                Description: <input name={`variations${idx}.description`} type="text" defaultValue={item.description} />
                </label>
                <label>
                Value: <input name={`variations${idx}.value`} type={checkInput(flagData.type)} defaultValue={item.value} readOnly={checkBoolean(flagData.type)} />
                </label>
                <br />
                </>
                        ))}
        </fieldset>
        <input type="hidden" name="variationCount" id="variationCount" value={flagData.variations.length} />
        <input type="hidden" name="flagType" id="flagType" value={flagData.type} />
